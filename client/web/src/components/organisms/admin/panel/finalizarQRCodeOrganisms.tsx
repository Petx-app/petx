...
} from "@/services/api/admin/panel/panelAdminService";
import { toast } from "react-toastify";
import { useEffect, useRef, useState } from "react";
import QRCode from "qrcode.react";

const FinalizarQRCodeOrganisms = ({ fetchRegistros, fetchState }) => {
  const [handleState, setHandleState] = useState<Boolean>(false);
  const [link, setLink] = useState<String>();
  const [uuid, setUuid] = useState<String>();
  const qrCodeRef = useRef();

  const url = `${process.env.NEXT_PUBLIC_FRONT_APP_API_URL}/qrcode?tag=`;
